...
const LeadLedger = ({ campaign }) => {
  ...
  const handleExport = () => {
    console.log(`[LEDGER] Exporting ${activeView} data to mission-ready Excel...`);
    ...
    if (activeView === "COMPANIES") {
      exportData = companies.map((c, i) => ({
        ...
        "Company Size": c.employee_count || "N/A",
      ...
      exportData = contacts.map((dm, i) => ({
        ...
        "LinkedIn Profile": dm.linkedin || "N/A",
    ...
    try {
      console.log(`[LEDGER] EXPORT INITIATED: ${exportData.length} records detected.`);
      
      const worksheet = XLSX.utils.json_to_sheet(exportData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, activeView);
      
      // Attempt 1: High-Fidelity Binary Blob (XLSX)
      const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
      console.log(`[LEDGER] Binary payload locked: ${excelBuffer.byteLength} bytes.`);
      ...
      console.log("[LEDGER] SUCCESS: XLSX Mission intelligence dispatched.");
    } catch (error) {
      console.error("[LEDGER] XLSX Critical Failure, switching to CSV recovery...", error);
      
      // Attempt 2: Native CSV Fallback (Bypasses library serialization)
      try {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-[11px] font-black text-slate-600 uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95"
          >
            <Download size={14} />
            Export
          </button>
                ...
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  {activeView === "COMPANIES" ? (
                    ...
                    <>
                      ...
                      <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest border-r border-slate-100 min-w-[120px]">LinkedIn</th>
                      <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest border-r border-slate-100 min-w-[150px]">Verified Email</th>
                      <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest min-w-[120px]">Active Status</th>
                    </>
                  )}
                </tr>
              </thead>
                  ...
                  >
                    ...
                    {activeView === "COMPANIES" ? (
                      <>
                        ...
                        <td className="px-6 py-5 border-r border-slate-100 min-w-[160px]">
                          <a href={ensureAbsoluteUrl(item.linkedin)} target="_blank" rel="noreferrer" className="text-brand-primary hover:underline transition-all text-[10px] font-bold truncate block max-w-[150px]">
                            {item.linkedin?.replace('https://', '').replace('www.', '') || "N/A"}
                          </a>
                        </td>
                        <td className="px-6 py-5 border-r border-slate-100 min-w-[160px]">
                          <a href={ensureAbsoluteUrl(item.website)} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-brand-primary hover:underline transition-all text-[10px] font-bold truncate block max-w-[150px]">
                        ...
                        <td className="px-6 py-5 border-r border-slate-100 min-w-[180px]">
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-wider bg-slate-100 px-2 py-1 rounded-md block whitespace-normal break-words">
